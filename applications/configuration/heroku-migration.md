...
Amvera позволяет указать окружение в [файле конфигурации](config-file.md) ``amvera.yml``. Если окружение не указано, подразумевается окружение ``docker`` (аналог стека ``Container`` в Heroku).
...
[Подробнее о том, что это значит](../environments/python-pip.md)
...
[Подробнее о том, что это значит](../environments/nodejs-server.md)
...
[Подробнее о том, что это значит](../environments/jvm-maven.md)
...
[Подробнее о том, что это значит](../environments/jvm-gradle.md)

Что-то другое

Скорее всего, мы пока не поддерживаем сборку вашего окружения нативно и вместо этого нужно использовать окружение ``docker``. Однако список поддерживаемых окружений со временем обновляется, актуальную информацию о поддерживаемых окружениях можно найти [здесь](../supported-env.md).
...
[Подробнее о том, что это значит](docker.md)
...
[Подробнее о развертывании образов Docker](docker.md)
...
[ Next Переменные и секреты ](variables.md) [ Previous Selenium и Chromedriver ](../environments/selenium-chromedriver.md)
